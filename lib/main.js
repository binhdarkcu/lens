var main = (function() { 
  ...
  function setZIndex(){
     ...
     //cTrue.setElement(getElementByID('big-img'));
   // cTrue.setHeight(HCanDefault * rHCan);
   // var wTrue = cTrue.getWidth();
    //var leftTrue =  getCenterLine() - (dCanDefault  * rDCan) - wTrue/2;
    // cTrue.setLeft(leftTrue);
    // cFalse.setElement(getElementByID('big-img'));
    // var angleStart2 =  getCenterLine() + (dCanDefault * rDCan) - wTrue/2;
    // cFalse.setHeight(HCanDefault * rHCan);
     //cFalse.setLeft(angleStart2);
      canvas.renderAll();
  }
...
function getDLenTrue(){
 ...
 // valueofA.addEventListener('keyup', function(){
 //    var dCanValue = parseInt(this.value);
...
//    });
 ...
 document.getElementById('html').onclick = function(e) {
    if(e.target.className != 'normal' && e.target.className != 'hover' && e.target != hLenValue && e.target.className != 'slider-track' && e.target.className != 'slider-handle min-slider-handle round') {
          
          hLenValue.value =  oldHLenValue;
    }
    if(e.target.className != 'normal' && e.target.className != 'hover' && e.target != valueofA && e.target.className != 'slider-track' && e.target.className != 'slider-handle min-slider-handle round') {
          
          valueofA.value =  oldValueofA;
    } 
    if(e.target.className != 'normal' && e.target.className != 'hover' && e.target != valueofF && e.target.className != 'slider-track' && e.target.className != 'slider-handle min-slider-handle round') {
 ...
 function resetCavas(){
  ...
  function setWhenChangeA(hChange,fchanged,dChanged){
     ...
     var wFFalse = getHeightCanFalse(hChange,dChanged,fchanged) * widthF / heightF + 10;
     ...
     if(parseInt(dChanged) == parseInt(fchanged)){
        ...
        var yEndPoint = calLine(xLenTop,yLenTop,xFFalse,yFFalse,getWidthLine());
         ...
         //lightTopright.set({'y1':0,'y2':yEndPoint});
   ...
   var hC = parseInt(getHeightCanFalse(hChange,dChanged,fchanged) * rHCan);
    ...
    cTrue.setWidth(wFTrue);
     cFalse.setWidth(wFFalse);
