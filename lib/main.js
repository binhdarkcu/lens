...
fControlb.onchange = function(){
 ...
 function resetCavas(){
  ...
  function setWhenChangeA(hChange,fchanged,dChanged){
    ...
    //var fLeftFalse = getCenterLine() + (dChanged * rDCan) - cFalse.getWidth()/2;
    //var fRightTrue = getCenterLine() - (dChanged * rDCan) - cTrue.getWidth()/2;
    //console.log(fRightTrue);
    //console.log(fLeftFalse);
   // cFalse.setLeft(getCenterLine() + (dChanged * rDCan) - cFalse.getWidth()/2);
    mcontrol.value = getmValue(dChanged,fchanged);
    //canvas.renderAll();
    //setWidthFalse(dChanged);
  }

 function setWidthFalse(dChanged){
    console.log(cTrue.getWidth());
   var fLeftFalse = getCenterLine() + (dChanged * rDCan) - cFalse.getWidth()/2;
   console.log('f:'+fLeftFalse);
   cFalse.setLeft(fLeftFalse);
  canvas.renderAll();
 }
